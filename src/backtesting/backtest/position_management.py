...
def process_positions(
    ...
    if is_long and high_price > position.get("highest_price", 0):
        ...
        position.get("lowest_price") is None or low_price < position["lowest_price"]
    ):
        position["lowest_price"] = low_price

    # Check for stop loss hit
    if (is_long and low_price <= position["stop_loss"]) or (
        not is_long and high_price >= position["stop_loss"]
    ):
        # Close at stop loss
    ...
    if (is_long and high_price >= position["take_profit"]) or (
        not is_long and low_price <= position["take_profit"]
    ):
    ...
    if risk_manager.should_use_trailing_stop(unrealized_pnl_pct):
        ...
        if (is_long and new_stop > position["stop_loss"]) or (
            not is_long and new_stop < position["stop_loss"]
        ):
...
def close_position(
    ...
    # Update risk manager
    risk_manager.update_trade_result({"realized_pnl": net_pnl, "status": "closed"})
