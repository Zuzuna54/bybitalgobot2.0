    ...
    if trades:
        ...
        if total_trades > 0:
            ...
            profit_factor = (
                ...
                if losing_trades > 0
                and abs(trades_df[trades_df["net_pnl"] < 0]["net_pnl"].sum()) > 0
                else float("inf")
            )
        else:
            win_rate = 0
...
def save_results(results: Dict[str, Any], output_dir: str) -> str:
    ...
    pd.DataFrame(results["trades"]).to_csv(
        os.path.join(base_path, "trades.csv"), index=False
    )

    # Save equity curve
    pd.DataFrame(results["equity_curve"]).to_csv(
        os.path.join(base_path, "equity_curve.csv"), index=False
    )
...
def generate_charts(base_path: str, results: Dict[str, Any]) -> None:
    ...
    # Update style using a compatible style
    try:
        # Try to use seaborn's style directly if available
        import seaborn as sns

        sns.set_style("darkgrid")
    except (ImportError, AttributeError):
        # Fall back to a standard matplotlib style if seaborn is not available
        try:
            plt.style.use("ggplot")
        except:
            # If that fails too, just use the default style
            pass
    ...
    if equity_data:
        ...
        plt.plot(
            equity_df["timestamp"], equity_df["equity"], label="Equity", linewidth=2
        )
        plt.plot(
            equity_df["timestamp"],
            equity_df["balance"],
        ...
        ax2.set_ylim(
            bottom=0,
            top=(
                max(equity_df["drawdown_pct"]) * 1.5
                if max(equity_df["drawdown_pct"]) > 0
                else 10
            ),
        )
        ax2.invert_yaxis()
    ...
    if results["strategy_metrics"]:
        ...
        win_rates = [
            results["strategy_metrics"][s]["win_rate"] * 100 for s in strategies
        ]
        total_pnls = [
            results["strategy_metrics"][s]["total_profit_loss"] for s in strategies
        ]
