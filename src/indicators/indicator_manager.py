...
class IndicatorManager:
    ...
    def __init__(self):
        ...
        self.indicator_registry: Dict[str, Type[BaseIndicator]] = (
    ...
    def add_indicator(
        self, name: str, indicator_type: str, params: Optional[Dict[str, Any]] = None
    ) -> None:
        """
    ...
    def apply_indicators(
        ...
        for name in names_to_apply:
            ...
            try:
                ...
                # Continue without raising to allow other indicators to be applied
                # The strategies will handle missing indicators gracefully
    ...
    def configure_default_indicators(self) -> None:
        ...
        self.add_indicator(
            "default_stochastic",
            "stochastic",
            {"k_period": 14, "k_slowing": 3, "d_period": 3},
        )
        ...
        # Volatility Indicators
        self.add_indicator(
            "default_bollinger", "bollinger", {"period": 20, "std_dev": 2.0}
        )
    ...
    def configure_strategy_indicators(
        ...
        if strategy_name == "ema_crossover":
            ...
            self.add_indicator(
                crossover_name,
                "ma_crossover",
                {"fast_period": fast_period, "slow_period": slow_period, "type": "ema"},
            )
            ...
            self.add_indicator(
                bb_name, "bollinger", {"period": bb_period, "std_dev": bb_std_dev}
            )
            self.add_indicator(rsi_name, "rsi", {"period": rsi_period})
            ...
            signal_length = strategy_config.get("parameters", {}).get(
                "signal_length", 9
            )
            ...
            vwap_period = strategy_config.get("parameters", {}).get(
                "vwap_period", "daily"
            )
            ...
            volume_confirmation = strategy_config.get("parameters", {}).get(
            ...
            self.add_indicator(
                fast_name, "ma", {"period": fast_ma, "type": ma_type.lower()}
            )
            self.add_indicator(
                slow_name, "ma", {"period": slow_ma, "type": ma_type.lower()}
            )
            self.add_indicator(
                crossover_name,
                "ma_crossover",
            ...
            keltner_period = strategy_config.get("parameters", {}).get(
                "keltner_period", 20
            )
            atr_period = strategy_config.get("parameters", {}).get("atr_period", 10)
            atr_multiplier = strategy_config.get("parameters", {}).get(
                "atr_multiplier", 2.0
            )
            adx_period = strategy_config.get("parameters", {}).get("adx_period", 14)
