...
from src.testing.backtest_engine import BacktestEngine
...
class TradingSystem:
    ...
    def _run_trading_loop(self, is_paper_trading: bool) -> None:
        ...
        logger.info(f"Timeframe: {timeframe}, Update interval: {update_interval} seconds")
            ...
            try:
                ...
                for symbol in symbols:
                    try:
                        ...
                        market_data = self.market_data.get_latest_data(symbol, timeframe, bars=100)
                        ...
                        # Apply indicators
                        market_data = self.indicator_manager.apply_indicators(market_data)
                        ...
                        if signals:
                            for signal in signals:
                                logger.info(f"Signal generated for {symbol}: {signal.signal_type.name} - strength: {signal.strength:.2f}")
                ...
                if datetime.now().minute % 15 == 0 and datetime.now().second < update_interval:
                    self._save_performance_metrics()
                
                # Sleep until next update
                logger.debug(f"Sleeping for {update_interval} seconds until next update")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Cryptocurrency Algorithmic Trading System")
