...
def process_trading_signal(
    ...
    if signal.signal_type == SignalType.NEUTRAL:
        logger.info(f"Neutral signal received for {signal.symbol} - no trade executed")
        return None
    ...
    symbol_info = api_client.get_symbol_info(signal.symbol)
    min_order_qty = float(symbol_info.get("minOrderQty", 0))
    ...
    if position_size <= 0:
        logger.warning(f"Position size calculation resulted in zero or negative value - skipping trade")
        return None
...
def execute_trade_entry(
    ...
    if simulate:
        ...
        trade.update_order(entry_order_id, {
    ...
    try:
        ...
        if "orderId" in entry_order_result:
            ...
            if order_status.get("orderStatus") == "Filled":
                ...
                tp_side = OrderSide.SELL if trade.side == OrderSide.BUY else OrderSide.BUY
