...
def execute_signal(
    ...
    for strategy_name in signal.metadata.get("contributing_strategies", [signal.metadata.get("strategy_name")]):
        if strategy_name in strategy_manager.strategies:
            strategy = strategy_manager.strategies[strategy_name]
            stop_loss_price = strategy.get_stop_loss_price(entry_price, is_long, current_data)
            break
    
    # If no stop loss was calculated, use a default percentage
    if stop_loss_price is None:
        atr_value = current_data.iloc[-1].get("atr")
        stop_loss_price = risk_manager.calculate_stop_loss(entry_price, is_long, atr_value)
    
    # Calculate take profit based on risk-reward ratio
    take_profit_price = risk_manager.calculate_take_profit(entry_price, stop_loss_price, is_long)
