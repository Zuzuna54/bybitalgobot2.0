...
class WebSocketService:
                    ...
                    if full_topic in self.public_callbacks:
                        for callback in self.public_callbacks[full_topic]:
                            try:
                                ...
                                logger.error(f"Error in callback for ticker topic {topic}: {e}")
                    return
                ...
                if topic in self.public_callbacks:
                    for callback in self.public_callbacks[topic]:
                        try:
                            ...
                            logger.error(f"Error in public callback for topic {topic}: {e}")
                ...
                if topic in self.private_callbacks:
                    for callback in self.private_callbacks[topic]:
                        try:
                            ...
                            logger.error(f"Error in private callback for topic {topic}: {e}")
    ...
    def _connect_public(self) -> None:
        ...
        try:
            ...
            logger.error("websocket-client module not installed. Install it with 'pip install websocket-client'")
    ...
    def _connect_private(self) -> None:
        ...
        if not self.connection_manager.api_key or not self.connection_manager.api_secret:
            logger.warning("No API credentials provided, skipping private WebSocket connection")
            return
    ...
    def subscribe_public(self, topics: List[str], callback: Callable[[Dict[str, Any]], None]) -> None:
        """
    ...
    def subscribe_private(self, topics: List[str], callback: Callable[[Dict[str, Any]], None]) -> None:
        """
    ...
    def subscribe_to_ticker(self, symbol: str, callback: Callable) -> None:
        ...
        if self.public_ws and self.public_ws.sock and self.public_ws.sock.connected:
            ...
            logger.warning(f"Cannot subscribe to {symbol} ticker: WebSocket not connected")