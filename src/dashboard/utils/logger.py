...
class DashboardLogger:
    ...
    def __init__(self, 
        ...
        self.name = name
        self.log_level = log_level or config.get("dashboard.log_level", DEFAULT_LOG_LEVEL)
        self.log_format = log_format or config.get("dashboard.log_format", DEFAULT_LOG_FORMAT)
        self.log_date_format = log_date_format or config.get("dashboard.log_date_format", DEFAULT_DATE_FORMAT)
        self.log_directory = log_directory or config.get_path("logs_dir") or DEFAULT_LOG_DIRECTORY
        self.log_to_console = log_to_console if log_to_console is not None else config.get_bool("dashboard.log_to_console", True)
        self.log_to_file = log_to_file if log_to_file is not None else config.get_bool("dashboard.log_to_file", True)
        self.max_file_size = max_file_size or config.get_int("dashboard.max_log_file_size", 10 * 1024 * 1024)
        self.backup_count = backup_count or config.get_int("dashboard.log_backup_count", 5)
        self.rotating_when = rotating_when or config.get("dashboard.log_rotating_when", "midnight")
        self.debug_mode = debug_mode if debug_mode is not None else config.get_bool("dashboard.debug_mode", False)
