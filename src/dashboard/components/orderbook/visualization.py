    ...
    if support_levels:
        for i, level in enumerate(support_levels[:3]):  # Show top 3 levels
            ...
            fig.add_annotation(
                ...
                showarrow=False,
                font=dict(
                    color=f"rgba(0, 128, 0, {opacity + 0.1})",
                    size=10
                ),
    ...
    if resistance_levels:
        for i, level in enumerate(resistance_levels[:3]):  # Show top 3 levels
            ...
            fig.add_annotation(
                ...
                showarrow=False,
                font=dict(
                    color=f"rgba(220, 53, 69, {opacity + 0.1})",
                    size=10
                ),
                bgcolor="rgba(255, 255, 255, 0.7)",
...
def render_support_resistance_levels(levels_data: Dict[str, Any]) -> html.Div:
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    html.Div(f"S{i+1}: {level:.4f}", className="support-level-badge"),
                    render_level_strength_indicator(
                        strength=1.0 - (i * 0.2),  # Decrease strength for each level
                        is_support=True
                    )
                ], className="level-item")
                for i, level in enumerate(support_levels[:5])  # Display top 5 levels
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    html.Div(f"R{i+1}: {level:.4f}", className="resistance-level-badge"),
                    render_level_strength_indicator(
                        strength=1.0 - (i * 0.2),  # Decrease strength for each level
                        is_support=False
                    )
                ], className="level-item")
                for i, level in enumerate(resistance_levels[:5])  # Display top 5 levels
            ], className="levels-container")
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    html.Div(f"{key.upper()}: {value:.4f}", 
                             className="vwap-level-badge",
                             style={"borderLeftColor": get_vwap_color(key)})
                ], className="level-item")
                for key, value in vwap_levels.items()
            ], className="levels-container") if vwap_levels else 
            html.Div("No VWAP data available", className="no-data-message")
        ], className="vwap-levels-section") if vwap_levels else html.Div(),
...
def create_level_confluence_chart(levels_data: Dict[str, Any]) -> dcc.Graph:
    ...
    for source in ['Volume', 'Price Action', 'VWAP']:
        ...
        for level in source_levels:
            ...
            hover_texts.append(f"{level_type}: {level['price']:.4f}<br>Strength: {strength:.1f}")
...
def render_execution_recommendations(recommendations: Dict[str, Any], risk_tolerance: float = 0.5) -> html.Div:
    """
    ...
    Returns:
        HTML Div with execution recommendations visualization
    """
    ...
    return html.Div([
        html.H5(f"{'Buy' if is_buy else 'Sell'} Recommendation", 
                className="recommendation-header",
                style={"color": "rgba(50, 171, 96, 1.0)" if is_buy else "rgba(220, 53, 69, 1.0)"}),
        
        # Risk tolerance indicator
        html.Div([
            html.Label("Risk Tolerance:", className="recommendation-label"),
            html.Div([
                html.Span(f"{risk_tolerance:.2f} - {'High' if risk_tolerance > 0.7 else 'Medium' if risk_tolerance > 0.3 else 'Low'}", 
                         style={"marginRight": "10px", "fontWeight": "bold", 
                                "color": "rgba(220, 53, 69, 0.8)" if risk_tolerance > 0.7 else 
                                         "rgba(255, 193, 7, 0.8)" if risk_tolerance > 0.3 else 
                                         "rgba(50, 171, 96, 0.8)"}),
                html.Div([
                    html.Div(
                        style={
                            "width": f"{risk_tolerance * 100}%",
                            "backgroundColor": "rgba(220, 53, 69, 0.8)" if risk_tolerance > 0.7 else 
                                              "rgba(255, 193, 7, 0.8)" if risk_tolerance > 0.3 else 
                                              "rgba(50, 171, 96, 0.8)",
                            "height": "8px",
                            "borderRadius": "4px"
        ...
        html.Div([
            ...
            html.Div([
                html.Span(f"{confidence_score:.2f} - {confidence_label}", 
                         style={"marginRight": "10px", "fontWeight": "bold", "color": confidence_color}),
                html.Div([
                    html.Div(
                        style={
        ...
        html.Div([
            html.Label("Recommended Order Type:", className="recommendation-label"),
            html.Div(
                order_type.upper(),
        ...
        html.Div([
            html.Label("Recommended Limit Price:", className="recommendation-label"),
            html.Div(
                f"{limit_price:.8f}" if limit_price else "N/A",
                className="limit-price-value"
            )
        ], className="recommendation-item") if order_type == 'limit' else html.Div(),
        
        # Order splitting recommendation
        html.Div([
            html.Label("Order Execution Strategy:", className="recommendation-label"),
            html.Div([
                html.Div(
                    f"Split into {num_parts} parts over {time_interval} seconds" if should_split else "Execute as single order",
                    className="execution-strategy-value"
                ),
                html.Div(
                    "TWAP/VWAP execution recommended" if should_split else "",
                    className="execution-substrategy-value"
                ) if should_split else html.Div()
            ])
        ], className="recommendation-item"),
        
        # Market impact (if available)
        html.Div([
            html.Label("Estimated Market Impact:", className="recommendation-label"),
            html.Div(
                f"{expected_impact:.2f}%" if expected_impact else "Unknown",
                className="impact-value",
                style={"color": "rgba(220, 53, 69, 0.8)" if expected_impact and expected_impact > 0.5 else "inherit"}
            )
        ], className="recommendation-item") if expected_impact is not None else html.Div(),
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    html.Span("Imbalance: ", className="condition-label"),
                    html.Span(
                        f"{market_conditions.get('imbalance', 0):.2f}",
                        style={"color": "rgba(50, 171, 96, 0.8)" if market_conditions.get('imbalance', 0) > 0 else 
                                       "rgba(220, 53, 69, 0.8)"}
                    )
                ], className="condition-item"),
                html.Div([
                    html.Span("Spread: ", className="condition-label"),
                    html.Span(f"{market_conditions.get('spread_pct', 0):.2f}%")
                ], className="condition-item"),
                html.Div([
                    html.Span("Position in Range: ", className="condition-label"),
                    html.Span(
                        f"{market_conditions.get('range_position', 0.5):.2f}",
                        style={"color": "rgba(50, 171, 96, 0.8)" if market_conditions.get('range_position', 0.5) < 0.3 else 
                                       "rgba(220, 53, 69, 0.8)" if market_conditions.get('range_position', 0.5) > 0.7 else
                                       "rgba(70, 130, 180, 0.8)"}
                    )
                ], className="condition-item") if 'range_position' in market_conditions else html.Div()
            ])
