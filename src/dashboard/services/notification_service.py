...
def create_toast(
    ...
    toast = dbc.Toast(
        ...
        header=[html.I(className=f"fas fa-{icon} me-2"), html.Strong(header)],
        dismissable=True,
        is_open=True,
        icon=color,
        duration=duration,
...
@callback_registrar(name="notification")
    ...
    def manage_notifications(
        ...
        if "n_dismiss" in triggered:
            ...
            if ctx.triggered:
                ...
                for trigger in ctx.triggered:
                    if (
                        isinstance(trigger["prop_id"], dict)
                        and trigger["prop_id"].get("type") == "notification-toast"
                    ):
                        indices.append(trigger["prop_id"].get("index"))

            # Remove dismissed notifications
            current_notifications = [
                n for n in current_notifications if n.get("id") not in indices
            ]
        ...
        if len(current_notifications) > 5:
            ...
            current_notifications.sort(
                key=lambda n: n.get("timestamp", 0), reverse=True
            )
...
def with_error_handling(callback_func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            result = (
                list(callback_func(*args, **kwargs))
            ...
            if result and len(result) > 0:
                ...
                result = [None] * (len(dash.callback_context.outputs) - 1) + [
                    error_notification
                ]
