...
class DashboardDataService:
    ...
    def _generate_sample_equity_history(self, days: int = 30) -> pd.DataFrame:
        ...
        # Create DataFrame
        df = pd.DataFrame(
            {"date": dates, "equity": equities, "drawdown_pct": drawdowns}
        )
    ...
    def _generate_sample_daily_returns(self, days: int = 30) -> pd.DataFrame:
        ...
        daily_returns = np.random.normal(
            0.001, 0.015, size=len(dates)
        )  # Mean 0.1%, std 1.5%

        # Create DataFrame
        df = pd.DataFrame({"date": dates, "return_pct": daily_returns * 100})
        df.set_index("date", inplace=True)
    ...
    def _generate_sample_trade_history(self, count: int = 20) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            trade = {
                ...
                "exit_time": (trade_time + timedelta(hours=4)).strftime(
                    "%Y-%m-%d %H:%M"
                ),
                "entry_price": price,
            ...
            if self.standalone_mode:
                ...
                if self.performance_tracker:
                    ...
                    if daily_returns is not None and not daily_returns.empty:
                        ...
                        cumulative_returns = (
                            daily_returns + 1
                        ).cumprod() * initial_equity
                        equity_data = pd.DataFrame({"equity": cumulative_returns})
                    ...
                    try:
                        total_return_pct = (
                            self.performance_tracker.get_total_return() * 100
                            if hasattr(self.performance_tracker, "get_total_return")
                            else 0
                        )

                        max_drawdown_pct = (
                            self.performance_tracker.get_max_drawdown() * 100
                            if hasattr(self.performance_tracker, "get_max_drawdown")
                            else 0
                        )

                        sharpe_ratio = (
                            self.performance_tracker.get_sharpe_ratio()
                            if hasattr(self.performance_tracker, "get_sharpe_ratio")
                            else 0
                        )
    ...
    def get_orderbook_data(
        self, symbol: Optional[str] = None, depth: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        try:
            if self.standalone_mode:
                ...
                if self.market_data:
                    ...
                    if symbol is None and hasattr(
                        self.market_data, "get_current_symbol"
                    ):
                    ...
                    if hasattr(self.market_data, "get_orderbook"):
                        ...
                        if orderbook:
                            # Use the DataTransformer to standardize the transformation
                            return data_transformer.transform_orderbook_data(
                                orderbook, depth
                            )

                    # Fall back to sample data if no orderbook found
                    logger.info(
                        f"No orderbook data found for {symbol}, using sample data"
                    )
    ...
    def _get_sample_orderbook_data(
        self, symbol: Optional[str] = None, depth: int = 10
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_sample_orderbook(
        ...
        try:
            if self.standalone_mode:
                ...
                if self.strategy_manager:
                    ...
                    # Get strategy data
                    if strategy_id is not None and hasattr(
    ...
    def _generate_sample_strategy_data(
        self, strategy_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        profit_factor = (
            (profitable_trades * avg_profit) / (losing_trades * avg_loss)
            if losing_trades > 0
            else 0
        )
            ...
            indicators.append(
                {
                    ...
                    "type": "oscillator",
                    "timestamp": (
                        datetime.now() - timedelta(minutes=np.random.randint(1, 10))
                    ).timestamp(),
                }
            )
            indicators.append(
            ...
            signals.append(
                {
                    ...
                    "timestamp": (
                        datetime.now() - timedelta(minutes=np.random.randint(1, 60))
                    ).timestamp(),
        ...
        return {
            ...
            "status": np.random.choice(
                ["active", "paused", "stopped"], p=[0.7, 0.2, 0.1]
            ),
        ...
        try:
            if self.standalone_mode:
                ...
                if self.market_data:
                    ...
                    if market_data:
                        # Transform the data
                        transformed_data = data_transformer.transform_market_data(
                            market_data
                        )
    ...
    def _generate_sample_market_data(
        ...
        # Generate sample indicators
        indicators = {
            "rsi": {"value": np.random.uniform(30, 70), "type": "oscillator"},
            "ma_50": {
        ...
        for i in range(24 * 60):  # 24 hours of 1-minute candles
            ...
            price_change = np.random.normal(0, 0.0005) + (
                0.0001 * (price - candle_price) / price
            )
