...
def create_aggregated_signal(
    ...
    unique_strategies = set(signal.metadata.get("strategy_name", "unknown") for signal in signals)
    
    # Check if we have enough concurrent strategies
    if len(unique_strategies) < min_concurrent_strategies:
        logger.debug(f"Not enough concurrent strategies for {signal_type} signal: "
    ...
    if weighted_aggregation:
        ...
        for signal in signals:
            ...
            strategy_weight = strategy_performance.get(strategy_name, {}).get("weight", default_weight)
    ...
    if strength < signal_threshold:
        logger.debug(f"Aggregated signal strength below threshold: {strength:.2f} < {signal_threshold}")
        return None
    
    # Create metadata including all contributing strategies
    contributing_strategies = [signal.metadata.get("strategy_name", "unknown") for signal in signals]
    ...
    logger.info(f"Created aggregated {signal_type.name} signal for {symbol} with strength {strength:.2f} "
