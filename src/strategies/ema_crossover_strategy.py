...
class EMACrossoverStrategy(BaseStrategy):
    ...
    def _init_indicators(self) -> None:
        ...
        logger.info(
            f"Initialized indicators for EMA Crossover strategy with fast={fast_period}, slow={slow_period}"
        )
    ...
    def generate_signals(self, data: pd.DataFrame) -> List[Signal]:
        ...
        volume_increasing = (
            latest_complete_candle["obv"] > latest_complete_candle["obv_ma"]
        )
        ...
        if crossover_value == 1 and volume_increasing:
            ...
            metadata = {
                "fast_ema": latest_complete_candle[
                    f"ema_fast_{self.parameters.get('fast_ema', 9)}"
                ],
                "slow_ema": latest_complete_candle[
                    f"ema_slow_{self.parameters.get('slow_ema', 21)}"
                ],
            ...
            logger.info(
                f"Generated BUY signal: {symbol} at {current_price} (strength: {volume_strength:.2f})"
            )
        ...
        # Generate sell signal on bearish crossover with volume confirmation
            ...
            signals.append(signal)
            logger.info(
                f"Generated SELL signal: {symbol} at {current_price} (strength: {volume_strength:.2f})"
            )
    ...
    def get_stop_loss_price(
        ...
        if is_long:
            ...
            return min(
                slow_ema, entry_price * 0.97
            )  # Use the lower of slow EMA or 3% below entry
        else:
            # For short positions, stop loss above the slow EMA
            slow_ema = data[f"ema_slow_{self.parameters.get('slow_ema', 21)}"].iloc[-1]
            return max(
                slow_ema, entry_price * 1.03
            )  # Use the higher of slow EMA or 3% above entry
