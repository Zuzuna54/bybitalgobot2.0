...
class BollingerBreakoutStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any], indicator_manager: IndicatorManager):
        ...
        self.rsi_overbought = config.get("rsi_overbought", 70)  # Exit long position if RSI exceeds this
        self.rsi_oversold = config.get("rsi_oversold", 30)    # Exit short position if RSI below this
    ...
    def generate_signals(self, data: pd.DataFrame) -> List[Signal]:
        ...
        if len(data) < self.bb_length + 5:
            logger.warning(f"Not enough data for Bollinger Breakout strategy (need at least {self.bb_length + 5} bars)")
            return signals
        ...
        if (bb_upper is None or bb_lower is None or rsi is None or 
            volume is None or volume_sma is None or atr is None):
        ...
        if (close_price > bb_upper and prev_close <= prev_bb_upper and 
            volume_condition and rsi > 50 and 
            consecutive_closes_above >= self.consecutive_closes_required):
            ...
            logger.info(f"Generated BUY signal for {symbol} at {close_price} (strength: {strength:.2f})")
        
        # Lower band breakout (sell signal)
        elif (close_price < bb_lower and prev_close >= prev_bb_lower and 
              volume_condition and rsi < 50 and 
              consecutive_closes_below >= self.consecutive_closes_required):
            ...
            logger.info(f"Generated SELL signal for {symbol} at {close_price} (strength: {strength:.2f})")
    ...
    def should_adjust_position(self, position_data: Dict[str, Any], current_data: pd.DataFrame) -> Tuple[bool, float]:
        ...
        avg_historical_width = np.mean(historical_width) if historical_width else current_band_width
