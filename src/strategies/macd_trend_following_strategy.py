...
class MACDTrendFollowingStrategy(BaseStrategy):
    ...
    def _init_indicators(self) -> None:
        ...
        if volume_name not in self.indicator_manager.indicators:
            self.indicator_manager.add_indicator(volume_name, "obv", {"ma_period": 20})

        # Store indicator names for this strategy
        self.strategy_indicators = [macd_name, volume_name]
        ...
        logger.info(
            f"Initialized indicators for MACD Trend Following strategy with fast={fast_length}, slow={slow_length}, signal={signal_length}"
        )
    ...
    def generate_signals(self, data: pd.DataFrame) -> List[Signal]:
        ...
        macd_cross_up = (
            latest_complete_candle["macd_line"] > latest_complete_candle["macd_signal"]
            and previous_candle["macd_line"] <= previous_candle["macd_signal"]
        )
        ...
        histogram_increasing = (
            latest_complete_candle["macd_histogram"] > previous_candle["macd_histogram"]
        )
        ...
        # Volume confirmation
        volume_increasing = (
            latest_complete_candle["volume"]
            > data["volume"].rolling(10).mean().iloc[-2]
        )
        ...
        if macd_cross_up and histogram_increasing and volume_increasing:
            ...
            histogram_strength = min(
                abs(latest_complete_candle["macd_histogram"]) / 0.01, 1.0
            )
            volume_ratio = (
                latest_complete_candle["volume"]
            ...
            logger.info(
                f"Generated BUY signal: {symbol} at {current_price} (strength: {signal_strength:.2f})"
            )
        ...
        # Generate sell signal on bearish crossover with decreasing histogram
            ...
            signals.append(signal)
            logger.info(
                f"Generated SELL signal: {symbol} at {current_price} (strength: {signal_strength:.2f})"
            )

        # Create and append exit signal
        signal = Signal(
    ...
    def get_stop_loss_price(
        ...
        if "atr" in data.columns:
            ...
            macd_strength = abs(
                data["macd_line"].iloc[-1] - data["macd_signal"].iloc[-1]
            )
    ...
    def should_adjust_position(
        ...
        if (histogram > 0 and previous_histogram < 0) or (
            histogram < 0 and previous_histogram > 0
        ):
